...
pub fn jump(engine: *Engine) void {
    var view = engine.getEcsRegistry().view(.{ Velocity, Speed, Movement, Collision }, .{});
...
pub fn accelerate(engine: *Engine) void {
    var view = engine.getEcsRegistry().view(.{ Velocity, Speed, Movement }, .{});
    var iter = view.entityIterator();
    while (iter.next()) |entity| {
        var velocity = view.get(Velocity, entity);
        const movement = view.getConst(Movement, entity);
...
pub fn beginMovement(engine: *Engine) void {
    var view = engine.getEcsRegistry().view(.{ Position, Velocity }, .{});
    var iter = view.entityIterator();
    while (iter.next()) |entity| {
        var velocity = view.get(Velocity, entity);
        velocity.x = 0;
